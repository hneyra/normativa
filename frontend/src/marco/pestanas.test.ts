...
/**
 * AC4, AC6 y AC7 sobre la mitad del marco que no necesita un navegador.
 *
 * Cerrar la primera de tres pestanas y comprobar cual queda activa es una afirmacion sobre
 * una lista, no sobre el DOM. Probarla montando React costaria un `render` por caso y diria
 * menos: lo que hay que ver es la lista de antes y la de despues.
 */
...
describe('el estado de partida', () => {
  it('sin hash, arranca con el panel de Normativa abierto y activo', () => {
  ...
  it('con hash, la seccion pedida ADEMAS tiene su pestana', () => {
    // El artboard fijaba `dest` desde el hash sin tocar `abiertas`: recargar sobre
    // `#cuadros` dejaba activa una seccion sin pestana en la barra —no se veia y no se podia
    // cerrar—. Aqui la pestana existe.
    expect(estadoInicial('nor-cuadros')).toEqual({
      abiertas: ['nor-panel', 'nor-cuadros'],
      activa: 'nor-cuadros',
      sucias: {},
      porCerrar: null,
    });
  ...
  it('si el hash pide el panel, no se abre dos veces', () => {
    expect(estadoInicial('nor-panel').abiertas).toEqual(['nor-panel']);
  });
});

describe('AC4 — abrir anade si no estaba, y si estaba solo activa', () => {
  it('un submodulo nuevo se anade al final', () => {
    const estado = con('fis-actas', 'coa-exp');

    expect(estado.abiertas).toEqual(['nor-panel', 'fis-actas', 'coa-exp']);
  ...
  it('abrir el que ya esta activo devuelve el MISMO estado, no uno igual', () => {
    // No es una optimizacion: el oyente de `hashchange` despacha un `abrir` por cada cambio
    // de hash y el hash lo escribe el efecto que sigue a la pestana activa. Con un objeto
...
describe('AC6 — el estado sin guardar', () => {
  ...
  it('sin ninguna pestana activa no se ensucia nada', () => {
    const vacio = reducir(
      reducir(estadoInicial(null), { tipo: 'cerrar', destino: 'nor-panel' }),
      { tipo: 'ensuciar' },
    );
  ...
  it('cerrar una pestana SUCIA pregunta en vez de cerrar', () => {
    ...
    expect(estado.abiertas, 'preguntar no cierra').toEqual(['nor-panel', 'fis-actas']);
  });
  ...
  it('cerrar una pestana LIMPIA no pregunta: cierra', () => {
    ...
    expect(estado.abiertas).toEqual(['nor-panel']);
  });
  ...
  it('seguir editando deja todo como estaba, la marca incluida', () => {
    ...
    expect(estado.porCerrar).toBeNull();
    expect(estado.abiertas).toEqual(['nor-panel', 'fis-actas']);
    expect(estado.sucias).toEqual({ 'fis-actas': true });
  });
...
describe('AC6 — cerrar la activa activa la vecina', () => {
  ...
  it('la de la DERECHA cuando se cierra la primera', () => {
    const tres = reducir(con('fis-actas', 'coa-exp'), { tipo: 'abrir', destino: 'nor-panel' });
    const estado = reducir(tres, { tipo: 'cerrar', destino: 'nor-panel' });
  ...
  it('cerrar una que NO es la activa no mueve la activa', () => {
    ...
    expect(estado.abiertas).toEqual(['nor-panel', 'coa-exp']);
    expect(estado.activa).toBe('coa-exp');
  });

  it('cerrar la ULTIMA deja el espacio vacio, y eso es lo correcto', () => {
    const estado = reducir(estadoInicial(null), { tipo: 'cerrar', destino: 'nor-panel' });
