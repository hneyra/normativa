...
/**
 * El filtro del arbol, aparte del componente porque es una funcion pura sobre el arbol y
 * porque su conteo tiene tres casos que conviene probar sin teclear en un `input`.
 *
 * La regla es la del artboard, y no es «se queda lo que casa»: **si casa el nombre del
 * MODULO, el modulo entra con sus cuatro submodulos**, aunque ninguno de ellos case. Buscar
 * «coactiva» tiene que ensenar lo que hay en Coactiva, no una lista vacia debajo de un
 * titulo que si casaba.
 *
 * Y el que casa se despliega solo: buscar y tener que abrir despues serian dos gestos para
 * uno.
 */
 ...
 *
 * Cadena vacia cuando no hay filtro —no hay nada que contar—, «Sin coincidencias» cuando no
 * casa nada, y el recuento en singular o plural en cada mitad por separado: «1 módulo · 4
 * submódulos» es una frase que ocurre.
 */
