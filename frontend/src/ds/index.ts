/**
 * El vocabulario visual de `normativa-web`: los ocho componentes base.
 *
 * Se importan desde aqui y no de su archivo, para que anadir el noveno sea una linea en
 * este indice y no una ronda de `import` por todas las pantallas.
 *
 * `export type` explicito en cada tipo: con `verbatimModuleSyntax` —que `tsconfig.base.json`
 * enciende y `andamiaje.test.ts` vigila— un tipo reexportado como valor se cuela en el
 * bundle y arrastra su modulo entero.
 */
