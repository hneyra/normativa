 ...
 * choca de frente con la etiqueta que `publicar-imagenes.yml` ya usa para las otras dos imagenes
 * de este repositorio: **el `sha` de este repositorio**. Una imagen por ambiente obliga a meter
 * el nombre del ambiente en la etiqueta, y entonces se pierden dos cosas a la vez: que la
 * etiqueta resuelva contra un `git log` —«que corre en la municipalidad» deja de tener
 * respuesta— y que lo verificado en la marcha blanca sea **el mismo artefacto** que se promueve
 * a produccion. Con una imagen por ambiente no se promueve nada: se vuelve a construir, y lo que
 * se despliega no es lo que se probo.
 *
 ...
 * todo arnes montara un `window.__KAMAYUK_NORMATIVA__` para dibujar una pantalla que no entra a
 ...
 * interfaz— que contesta 200 con el `index.html` dentro. Es el «200 que miente» que
 * `api/cliente.ts` ya nombra para la API, aplicado a la puerta de identidad: por eso una cadena
 * en blanco cuenta como ausencia.
 */
 ...
 *
 * `kamayuk-backoffice` y `localhost:8181` son los que hacen que `yarn dev` entre por la puerta
 * sin configurar nada. El puerto es el que `infrastructure/despliegue/.env.ejemplo` documenta
 * para Keycloak menos el desplazamiento de esta instalacion; quien lo tenga en otro sitio lo
 * pone en `VITE_KAMAYUK_OIDC_REALM`, que es el escalon de arriba.
 */
