...
/**
 * **El mando de preferencias: los dos ejes, y que la eleccion se recuerde** (#57, AC 6).
 *
 * Calcada de `rentas/frontend/src/preferencias/MandoDeTema.test.tsx@ac379ac`, sin el `t()` —el i18n
 * es #60— y con el prefijo de este sistema.
 *
 ...
 * `prefers-color-scheme`. Eso se mide en el navegador, y es de #61.
 */

/** Un prefijo de prueba, y no `kamayuk.normativa`: asi no se pisa con lo que el resto de la suite
 * deje puesto en `localStorage` ni con el `ProveedorDeTema` de `src/aplicacion.tsx`. */
const PREFIJO = 'kamayuk.prueba';

function montar() {
  return render(
    <ProveedorDeTema
      configuracion={{ identidadPorOmision: 'institucional', prefijoDeClaves: PREFIJO }}
    >
...
describe('el mando de preferencias', () => {
  ...
  it('por omision: la identidad del servicio, y NINGUN modo', () => {
    ...
    // Ausente y no «claro»: es lo que devuelve el mando al sistema operativo.
  ...
  it('elegir un modo lo estampa, y volver a «El del sistema» lo QUITA', () => {
    ...
    // Quitado, no puesto en claro: sin esto, quien devuelve el mando al equipo se quedaria clavado
    // en el modo que tuviera puesto al hacerlo.
    expect(raiz().hasAttribute('data-modo')).toBe(false);
  ...
  it('las dos claves llevan el prefijo del sistema: dos interfaces del mismo origen no se pisan', () => {
    montar();
    fireEvent.click(screen.getByRole('radio', { name: 'Sepia' }));
    // `/normativa/` y `/rentas/` se sirven del mismo origen y comparten `localStorage`. Con una
    // clave pelada —`tema`—, cambiar el tema aqui se lo cambiaria a la otra.
