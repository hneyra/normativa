 ...
 *
 * Una rama muerta **no se ve**. Si `PanelDeModulos` conservara `panelVar` con el valor `'a'`
 * fijo y sus tres ramas debajo, la pantalla se dibujaria exactamente igual, todas las
 * pruebas del marco seguirian verdes y el `aside` mediria sus 252 px. Lo unico que cambiaria
 * es que habria doscientas lineas que nadie puede poner en rojo — y a la primera pantalla
 * que alguien anada, la variante `c` volvera a ensenarse en una revision como si fuera una
 * opcion viva.
 *
 * Asi que esto se verifica como se verifica «ningun `SET SESSION`» en el backend: **leyendo
 * el codigo fuente**, no ejecutandolo. Es la misma forma de regla que la undecima —un `JOIN`
 * que cruza la frontera tampoco deja huella— y por el mismo motivo.
 *
 * <h2>Y la medida</h2>
 *
 * Los 292 px eran de la variante `c`, que necesitaba sitio para el riel de iconos MAS la
 * lista de submodulos al lado. Con solo la `a`, el panel mide 252. Un panel de 292 no da
 * error: da cuarenta pixeles de mas en cada pantalla del sistema, para siempre, y nadie sabe
 * de donde salieron.
 */
 ...
 *
 * `sinComentarios` —el de `tokens.ts`— quita solo los de bloque, porque nacio para CSS y en
 * CSS no hay otros. Aqui hace falta quitar tambien los de linea: los comentarios de este
 * marco NOMBRAN las tres variantes para explicar por que no estan, y un `// la cola ya no
 * depende de hayCola` pondria rojo justo a la prosa que documenta la decision.
 *
 * Es un recorte y se dice: un `//` dentro de una cadena —una URL, por ejemplo— se llevaria
 * por delante el resto de la linea. En `src/marco/` no hay ninguna, y si la hubiera el
 * efecto seria un falso VERDE de esa linea, nunca un rojo.
 */
...
const RASTROS: ReadonlyArray<readonly [string, string]> = [
  ...
  ['datosVar', 'el segundo conmutador del prototipo: con que datos se dibuja'],
  ['datosOpciones', 'sus dos botones'],
];
...
describe('AC2 — el panel mide 252 px, que es la medida de la variante A', () => {
  ...
  it('«.kn-marco__panel» declara 252 px', () => {
    expect(css).toMatch(/\.kn-marco__panel\s*\{[^}]*flex:\s*0 0 252px/);
    expect(css).toMatch(/\.kn-marco__panel\s*\{[^}]*width:\s*252px/);
  });
  ...
  it('y el 292 tampoco esta escondido en el codigo del marco', () => {
    // El artboard lo escribia en `asideStyle`, o sea en JavaScript y no en CSS. Mirar solo la
    // hoja habria dejado pasar justo la forma en la que existia.
    const conLaMedida = archivosDelMarco.filter((nombre) =>
      sinNingunComentario(leer(join(MARCO, nombre))).includes('292'),
    );

    expect(conLaMedida).toEqual([]);
  });
});
...
describe('AC2 — la cola de trabajo se muestra siempre', () => {
  it('no cuelga de ninguna condicion en el panel', () => {
    ...
    const cola = panel.slice(panel.indexOf('kn-marco__cola'));

    // `hayCola` era `v !== 'c'`. Lo que se comprueba aqui es que el bloque de la cola no esta
    // detras de un `&&` de JSX, que es como se escribe un condicional en este arbol.
    const antesDeLaCola = panel.slice(0, panel.indexOf('<div className="kn-marco__cola">'));
