...
import { IDENTIDADES, ProveedorDeTema } from '@kamayuk/ui';

/**
 * **El enlace con `kamayuk-lib` resuelve de verdad** (`rentas`#74).
 *
 * **Calcado de `rentas/frontend/verificaciones/` en `ac379ac`** (`normativa`#55), con una prueba mas
 * al final: la de los dos paquetes que `rentas` importa desde `src/` y aqui tambien —`@kamayuk/shell`
 * y `@kamayuk/ui`—, que en `rentas` no estan en este archivo porque los ejercen sus pantallas.
 *
 ...
 *
 * <h2>Y desde `rentas`#75, `src/` tambien puede</h2>
 *
 * Este archivo nacio importando desde `verificaciones/` y no desde `src/` porque el destino del
 * `link:` quedaba fuera del contexto de Docker. `rentas`#75 lo cerro con un contexto con nombre de
...
describe('y los dos que `src/aplicacion.tsx` importa tambien resuelven (`normativa`#55)', () => {
  it('@kamayuk/shell y @kamayuk/ui publican lo que la aplicacion monta', () => {
    // Que el `Armazon` se DIBUJE —con un solo React— lo mide `src/aplicacion.test.tsx`. Aqui solo
    // que los dos paquetes cruzan el enlace y traen lo que se les pide por nombre.
    expect(typeof Armazon).toBe('function');
    expect(typeof ProveedorDeTema).toBe('function');
    // Y la identidad que `aplicacion.tsx` pide por omision existe en la libreria: un nombre que no
    // estuviera dejaria `data-tema` sin reglas y la pantalla sin colores, sin un solo error.
    expect(IDENTIDADES).toContain('institucional');
  });
});
