...
/**
 * **El puerto del arnes sale del arbol de trabajo, y si esta ocupado se dice** (#61, AC 2).
 *
 * **Calcada de `rentas/frontend/verificaciones/el-puerto-del-arnes-sale-del-arbol.test.ts@ac379ac`**
 * (`rentas`#148), con las rutas de este repositorio.
 *
 * <h2>Que defecto cierra, y como se demostro alli que muerde</h2>
 *
 * El original: `playwright.config.ts` escribia el 4173 en tres sitios. Con seis worktrees a la
 * vez, `yarn e2e` de una rama media el bundle de otra —paso al cerrar `rentas`#140, y dio
 * `Received: 0`—. Las pruebas de aqui son las que se ponen rojas si el puerto vuelve a ser una
 * constante, si deja de depender del arbol, o si la comprobacion de ocupacion mira una sola cara
 * de `localhost`.
 *
 * Y aqui no es historia ajena: la epica #47 reparte los issues de `normativa` en worktrees de
 * `/home/jorge/ws/.relevo/<id>/normativa/` que corren a la vez, y en esta maquina hay ademas otras
 * tres interfaces del producto con su propio arnes.
 *
 * <h2>Por que hay una prueba que abre sockets</h2>
 *
 * Porque el modo de fallo que se midio es exactamente ese: `vite preview` escucha en `[::1]` y NO
 * en `127.0.0.1` —vuelto a medir en este arbol con `ss -ltnp`: `LISTEN [::1]:4699`, y un `curl` a
 * `127.0.0.1:4699` no conecta—, asi que una comprobacion que solo mirase la cara IPv4 diria
 * «libre» de un puerto que no lo esta. Eso no lo puede decir una prueba que no abra un socket.
 */
 ...
 * defecto de `rentas`#140 y por que el puerto no es una constante. Borrar esa historia para que
 ...
 *
 * Las cuatro primeras son las interfaces del producto en esta maquina; las dos ultimas, dos
 * worktrees de la epica #47 que se diferencian en un caracter — que es el caso que obliga a que la
 * derivacion sea un hash y no una suma casera.
 */
const ARBOLES = [
  '/home/jorge/ws/rentas/frontend',
  '/home/jorge/ws/catastro/frontend',
  '/home/jorge/ws/caja/frontend',
  '/home/jorge/ws/normativa/frontend',
  '/home/jorge/ws/.relevo/normativa-60/normativa/frontend',
  '/home/jorge/ws/.relevo/normativa-61/normativa/frontend',
];
...
describe('el puerto del arnes', () => {
  it('EL CENTINELA: `playwright.config.ts` no escribe NINGUN puerto a mano', () => {
    // Esta es la prueba que se pone roja el dia que alguien vuelva a fijar el 4173 —o cualquier
    // otro— en el config. No comprueba un numero: comprueba que no haya numero.
  ...
  it('y `yarn e2e` lo comprueba ANTES de arrancar Playwright', () => {
    // El orden es la mitad de la guarda. Medido en `rentas`#148 con un intruso en el puerto
    // derivado: puesta dentro de `webServer.command`, la comprobacion llega tarde —Playwright ya
    // ha dicho lo suyo, «… is already used», que no nombra ni el proceso ni el directorio—.
  ...
  it('y el navegador se instala aparte: `@playwright/test` trae el arnes, no Chromium', () => {
    const guion = JSON.parse(readFileSync(join(AQUI, '..', 'package.json'), 'utf8')) as {
      scripts: Record<string, string>;
    };
    expect(guion.scripts['e2e:navegador']).toContain('playwright install chromium');
  });
  ...
  it('y el del mismo arbol es el mismo siempre: es lo que se puede nombrar al depurar', () => {
    // Un puerto libre de verdad daria otro en cada corrida —medido en `rentas`: diez sondeos, diez
    // puertos—, y entonces un `vite preview` colgado de ayer no estorbaria a nadie nunca, que es
    // como se acumulan tres en la misma maquina sin que nadie se entere.
  ...
  it('el de `normativa` no choca con el de sus tres hermanas, que es para lo que existe', () => {
    // La medicion que el issue #61 escribio, vuelta a hacer aqui: 4195, 5068, 4591 y 4686.
    expect(ARBOLES.slice(0, 4).map(puertoDerivadoDe)).toEqual([4195, 5068, 4591, 4686]);
  });

  it('en CI es el de siempre, porque alli solo hay un arbol', () => {
    expect(
      elegirElPuerto({ raiz: '/home/runner/work/normativa/normativa/frontend', ci: true }),
    ).toEqual({
      puerto: 4173,
  ...
  it('fuera de CI NUNCA es el de CI, aunque el arbol este en la misma ruta', () => {
    expect(
      elegirElPuerto({ raiz: '/home/runner/work/normativa/normativa/frontend', ci: false }).puerto,
    ).not.toBe(4173);
  });
  ...
  it('el de ESTE arbol es el que lleva la URL con la que se mide', () => {
    expect(URL_DEL_ARNES).toBe(`http://localhost:${String(PUERTO)}/normativa/`);
    // `/normativa/` es la `base` de `vite.config.ts`: con otra, `preview` contesta 404 en la raiz
    // y el arnes esperaria 120 s a un servidor que si esta.
    const base = /base: '([^']+)'/.exec(readFileSync(join(AQUI, '..', 'vite.config.ts'), 'utf8'));
...
describe('la comprobacion de que el puerto esta libre', () => {
  it('ve un servidor que solo escucha en `[::1]`, que es como escucha `vite preview`', async () => {
    // ESTE es el rojo que importa: mirando solo 127.0.0.1, `estaOcupado` devuelve `false` de un
    // puerto en el que `vite preview` no va a poder abrir. Vuelto a medir en este arbol: el
    // `preview` de aqui abrio en `[::1]:4699` y `curl http://127.0.0.1:4699/` no conecto.
