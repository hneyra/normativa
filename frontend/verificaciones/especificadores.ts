...
// `remedioDelEnlace` se mudo a `remedio.mjs` en `rentas`#137: lo necesita `eslint.config.js`, que
 ...
 *
 * **Calcado de `rentas/frontend/verificaciones/` en `ac379ac`** (`normativa`#55). Llega como apoyo de
 * las tres guardas de Tailwind, que alcanzan la hoja de `@kamayuk/ui` por aqui. Su propia prueba en
 * `rentas` —`los-import-resuelven-por-exports.test.ts`— no entra en este issue.
 *
 ...
 *
 * Medido quitando `"./estilos.css"` del `exports` de `@kamayuk/ui` —exactamente lo que `rentas`#24 acaba de
 * hacer con `"./fuentes"`—:
 *
 ...
 * —y ese mensaje ya esta escrito una vez, en `enlace.ts` (`rentas`#113)—. Con el clon puesto, lo que falta
 ...
 *
 * Las dos cosas dan rojo; solo una dice donde. Como `const`, la rotura de `rentas`#138 —quitar
 ...
 * defecto de `rentas`#113 otra vez—. Definida despues de todo lo que el mensaje usa, no puede repetirse.
 */
