...
/**
 * **El mensaje que nombra el `git clone` llega a salir** (`rentas`#113).
 *
 * **Calcado de `rentas/frontend/verificaciones/` en `ac379ac`** (`normativa`#55). Solo cambia el
 * nombre del directorio de la raiz de mentira: `normativa` en vez de `rentas`.
 *
...
/**
 * Una raiz de frontend que no existe, con la disposicion de la de verdad: `…/normativa/frontend`,
 * para que `../../kamayuk-lib` caiga al lado de «normativa» y no en cualquier sitio.
 */
const RAIZ_SIN_HERMANO = join(tmpdir(), 'kamayuk-ws-de-mentira', 'normativa', 'frontend');
  ...
  it('no hay ni un `require.resolve` a pelo', () => {
    ...
    expect(
      ...
      'Un `resolve` sin envolver vuelve a reventar antes del mensaje que nombra el `git clone`,\n' +
        'y el rojo vuelve a ser «Cannot find module» dentro de `vite.config.ts` (`rentas`#113).',
    ).toEqual([]);
  });
...
describe('con el hermano en su sitio, resuelve', () => {
  it('los cinco enlaces se resuelven, y los que piden algo lo piden de verdad', () => {
