...
/**
 * **Cada `@import '@kamayuk/…'` del codigo resuelve POR EL `exports` de su paquete** (`rentas`#138).
 *
 * **Calcado de `rentas/frontend/verificaciones/` en `ac379ac`** (#55), sin una linea de codigo
 * distinta. Los `#N` de la prosa son de `rentas`, que es donde se midio; `docs/agent/HISTORY.md`
 * de este repositorio lleva la fila de #55 y no la de aquel.
 *
 ...
 * —exactamente lo que `kamayuk-lib`#24 hizo con `"./fuentes"`—:
 *
 ...
 * arregla. El razonamiento entero y sus cifras estan en el `docs/agent/HISTORY.md` de `rentas`, fila de su #138.
 *
...
describe('los `@import` de `@kamayuk/*` pasan por el `exports`', () => {
  ...
  it('LA MUESTRA: un subcamino que existe pero no se publica sale rojo', () => {
    ...
    // escribe `enlace.ts`, una sola vez, y nombra el `git clone` (`rentas`#113).
