...
/**
 * **El puerto del arnes sale del ARBOL, no de una constante** (#61).
 *
 * **Calcado de `rentas/frontend/puerto-del-arnes.mjs@ac379ac`** (`rentas`#148), con una sola cosa
 * cambiada: la `base` de la aplicacion, que aqui es `/normativa/`. Todo lo que este archivo
 * afirma se midio en aquel repositorio y se cita como suyo; lo que se midio AQUI, y con las rutas
 * de esta maquina, esta en {@link puertoDerivadoDe}.
 *
 * <h2>Lo que pasaba alli, y esta medido</h2>
 *
 ...
 * mide el bundle de otra. Ocurrio al cerrar `rentas`#140: la corrida midio el `dist/` del arbol
 * vecino y dio `Received: 0`, un rojo que no dice nada del puerto y se parece a un defecto
 * propio. Mientras se escribia aquello, `ss -ltnp` decia:
 *
 *     LISTEN [::1]:4173  users:(("node",pid=889605))
 *       → /home/jorge/ws/rentas-143/frontend  vite preview --port 4173 --strictPort
 *
 * O sea: el 4173 no estaba libre para nadie mas, y el arnes de aquel arbol habria medido
 * aquel bundle.
 *
 * <h2>Y por que este repositorio nace ya con ello</h2>
 *
 * Porque el choque no es hipotetico aqui: en esta maquina hay **cuatro interfaces** del producto
 * —`rentas`, `catastro`, `caja` y `normativa`— y la epica #47 reparte los issues de `normativa` en
 * worktrees de `/home/jorge/ws/.relevo/<id>/normativa/`, que corren a la vez. Con el 4173 escrito,
 * dos de ellos midiendo a la vez es un verde sobre el bundle del otro.
 *
 ...
 * 2. **Uno derivado de la ruta del arbol**, que es lo que hay aqui. Las ocho rutas de aquella
 ...
 * que antes de `rentas`#148, y una variable menos de la que dudar cuando un arnes falle en la nube.
 */
 ...
 * Son 999 puertos, y si dos arboles cayeran en el mismo lo dice `comprobarQueElPuertoEstaLibre`
 * nombrando quien lo tiene, y no un `Received: 0`.
 */
const PRIMERO = 4174;
const ULTIMO = 5172;
 ...
 *
 * SHA-256 y no una suma casera: `normativa-60` y `normativa-61` se diferencian en un caracter, y
 * un hash debil los manda al mismo sitio.
 *
 * **Medido AQUI, con esta misma funcion y las rutas de verdad de esta maquina** (#61):
 *
 * | Arbol | Puerto |
 * |---|---|
 * | `/home/jorge/ws/rentas/frontend` | 4195 |
 * | `/home/jorge/ws/catastro/frontend` | 5068 |
 * | `/home/jorge/ws/caja/frontend` | 4591 |
 * | `/home/jorge/ws/normativa/frontend` | **4686** |
 * | `/home/jorge/ws/.relevo/normativa-61/normativa/frontend` | **5012** |
 *
 * Ninguno coincide con otro ni con el 4173 de `vite preview` ni con el 5173 de `yarn dev`. O sea
 * que la derivacion de `rentas` ya separa a `normativa` de sus tres hermanas sin que nadie tenga
 * que repartir puertos a mano — y por eso el puerto propio **no se escribe en ningun sitio**.
 *
 ...
 *
 * `/normativa/` es la `base` de `vite.config.ts`: ADR-0030 §2 pone el sistema delante de la ruta.
 * Con otra, `vite preview` contesta 404 en la raiz y el arnes esperaria 120 s a un servidor
 * que si esta. Que las dos sigan diciendo lo mismo lo comprueba
 * `verificaciones/el-puerto-del-arnes-sale-del-arbol.test.ts` leyendo `vite.config.ts`.
 */
export const URL_DEL_ARNES = `http://localhost:${PUERTO}/normativa/`;
...
export async function comprobarQueElBundleServidoEsElMio() {
  ...
  throw new Error(
    `Lo que se sirve en ${URL_DEL_ARNES} NO es el bundle de este arbol.\n\n` +
      ...
      'Alguien ocupo el puerto mientras este arbol construia. Medir eso es lo que `rentas`#148 cerro:\n' +
 ...
 * que faltaba en `rentas`#140: sin ella, una corrida no deja escrito en que puerto midio.
 */
