...
import { AUTORIZACION, DESCUBRIMIENTO, REALM_POR_OMISION } from './instalacion.ts';

/**
 * **Sin emisor levantado, la pagina NO se queda en blanco** (#61, AC 3; `rentas`#112, `normativa`#57).
 *
 * <h2>Lo que se midio antes de escribir esto, en `rentas`</h2>
 *
 ...
 * siempre —un `setTimeout`, por ejemplo— cerraria el defecto y abriria un parpadeo en el camino
 ...
 * `location.assign` es un espia y las dos son la misma — que es exactamente el hueco que
 * `src/arranque.ts` y `src/puerta/` tenian sin arnes que lo cerrara.
 *
 ...
 *
 * <h2>Y aqui NO se agota el tope de idas</h2>
 *
 * Los demas caminos de este arnes montan la aplicacion sembrando `…pkce.idas` (ver
 * `instalacion.ts`). Este no: lo que mide es precisamente la ida, asi que la puerta tiene que
 * estar entera.
 */

/**
 * **La paleta contra la que se mide el aviso es `institucional/claro`.**
 *
 * `ProveedorDeTema` envuelve tambien esta capa —a proposito: quien eligio sepia u oscuro no debe
 * encontrarse el aviso de averia con la paleta de otro—. Los dos colores que este archivo clava
 * son los de `diseno/normativa-tokens.css`, o sea la combinacion clara, asi que el eje se declara
 * en vez de heredarse. El motivo largo, con la medicion que lo sostiene, en `se-ve.spec.ts`.
 */
...
test('la puerta que NO contesta: la aplicacion monta y dice quien fallo y en que URL', async ({
  ...
  const texto = await page.locator('#raiz').innerText({ timeout: 15_000 });
  ...
  expect(texto).toContain(REALM_POR_OMISION);
...
test('y el aviso SE VE: pintado con el rojo del artboard, no sin estilo', async ({ page }) => {
  ...
  await expect(aviso).toHaveAttribute('data-porque', 'no-contesto');

  // `bg-mal-fondo` y `text-mal-tinta` son clases del artboard, y una clase escrita no es una regla
  // emitida: `tailwind-emite-las-clases.test.ts` barre `src/pantallas` y la libreria, asi que esto
  // se mide aqui.
  const titulo = aviso.locator('p').first();
  const tinta = await titulo.evaluate((e) => getComputedStyle(e).color);
  expect(tinta, 'el aviso no esta pintado con la tinta de error del artboard').toBe(
    'rgb(143, 42, 23)',
  );
  const fondo = await titulo.evaluate((e) =>
...
test('la puerta que SI contesta: se va a ella y NO monta nada', async ({ page }) => {
  ...
  expect(
    dibujado,
    'se dibujo algo despues de mandar el navegador a la puerta: es el parpadeo que no puede volver',
  ).toBe(0);
});
