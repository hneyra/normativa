...
-- ==========================================================================
--  2. FUNCIONES
--  Las cuatro que quedan son TODAS de disparador, y las seis de este archivo
--  las usan: un disparador sin su funcion no protege nada,
--  y una funcion sin su disparador es codigo muerto que ademas MIENTE sobre
--  lo que este esquema contiene.
--
--  P5B RETIRO CINCO QUE EL GENERADOR HABIA ARRASTRADO, y conviene decir por
--  que se fueron: `nombre_normalizado` (la columna generada de `via`),
--  `verificar_participacion_no_excede` y `verificar_titularidad_no_excede`
--  son de `catastro`; `declaracion_jurada_estado_es_terminal`, de `rentas`;
--  y `valuacion_de_conjunto_sellado_es_inmutable` cuelga de `arancel`, que
--  se queda en `catastro` (D-N4). Ninguna tenia aqui un disparador que la
--  usara, y la de participacion consultaba `participacion_comun` -una tabla
--  de otro sistema-, asi que la caza el escaner de frontera: era un cruce
--  de sistema escrito en el propio esquema.
-- ==========================================================================
