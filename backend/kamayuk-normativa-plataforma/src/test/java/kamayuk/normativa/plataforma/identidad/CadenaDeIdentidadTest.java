package kamayuk.normativa.plataforma.identidad;
...
import kamayuk.normativa.esquema.DatosDePrueba;
import kamayuk.normativa.plataforma.ConfiguracionDeTenant;
import kamayuk.normativa.plataforma.SeguridadWeb;
...
// kamayuk-normativa-aplicacion (application.yaml), no el comportamiento por omision del starter.
...
class CadenaDeIdentidadTest {
    ...
    class Accesos {
        ...
        @Test
        @DisplayName("con el claim de A, se ven los grupos de A")
        void conElClaimDeAseVenLosDeA() throws Exception {
            ...
            assertThat(respuesta.body()).contains("Normativa A");
            assertThat(ControladorDePrueba.LLAMADAS.get()).isEqualTo(1);
        }

        @Test
        @DisplayName("con el claim de A no se ve ni un grupo de B, y al reves tampoco")
        void unaMunicipalidadNoVeLaOtra() throws Exception {
            ...
            assertThat(deA).contains("Normativa A").doesNotContain("Normativa B");
            assertThat(deB).contains("Normativa B").doesNotContain("Normativa A");
        }
        ...
        void elEncabezadoHostilNoGana() throws Exception {
            ...
            assertThat(respuesta.body())
                    ...
                    .contains("Normativa A")
                    .doesNotContain("Normativa B");
        }
    }
    ...
    /**
     * Lee {@code grupo}, que es una tabla de tenant con RLS. Se elige esa y no
     * `conjunto_parametros` porque el nombre del grupo DISTINGUE a las dos municipalidades
     * sembradas, y el ejercicio del conjunto no: una asercion sobre una columna que vale lo mismo
     * en las dos pasaria en verde con la fuga dentro.
     *
    ...
    static class ControladorDePrueba {
        ...
        static final String RUTA = "/normativa/api/v1/prueba/conjuntos";
        ...
        public List<String> direcciones() {
            ...
            return plantilla.queryForList("SELECT nombre FROM grupo ORDER BY nombre", String.class);
        }
    }
}
