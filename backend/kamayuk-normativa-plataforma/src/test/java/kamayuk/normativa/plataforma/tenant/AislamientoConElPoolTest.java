package kamayuk.normativa.plataforma.tenant;
...
import kamayuk.normativa.esquema.ContextoDeTenant;
...
class AislamientoConElPoolTest {
    ...
    void elContextoNoSobreviveALaDevolucionDeLaConexion() {
        try (Entorno entorno = new Entorno(1)) {
            long vistos = entorno.enContextoDe(municipalidadA, this::conjuntos);
            assertThat(vistos).as("con contexto, la municipalidad A ve su conjunto").isEqualTo(1);

            // Misma conexion fisica: el pool tiene una sola.
            Throwable fallo = entorno.sinContexto(this::conjuntos);
    ...
    void dosMunicipalidadesSobreLaMismaConexionNoSeVenEntreSi() {
        try (Entorno entorno = new Entorno(1)) {
            assertThat(entorno.enContextoDe(municipalidadA, this::conjuntosDeA)).isEqualTo(1);
            assertThat(entorno.enContextoDe(municipalidadB, this::conjuntosDeA))
                    .as("la municipalidad B no ve el conjunto de A ni reutilizando su conexion")
                    .isZero();
    ...
    void sinElGuardiaLaConexionContaminadaSeReutiliza() {
        ...
        try (Entorno entorno = new Entorno(1, false)) {
            ...
            long vistosSinContexto = entorno.enContextoDe(null, this::conjuntos);
    ...
    private long medir(Entorno entorno, int repeticiones) {
        ...
        for (int i = 0; i < repeticiones; i++) {
            entorno.enContextoDe(municipalidadA, this::conjuntos);
        }
        return System.nanoTime() - inicio;
    }

    private long conjuntos(JdbcTemplate jdbc) {
        Long total = jdbc.queryForObject("SELECT count(*) FROM conjunto_parametros", Long.class);
        return total == null ? -1 : total;
    }

    private long conjuntosDeA(JdbcTemplate jdbc) {
    ...
    private long contarPor(JdbcTemplate jdbc, long municipalidadId) {
        Long total =
                jdbc.queryForObject(
                        "SELECT count(*) FROM conjunto_parametros WHERE municipalidad_id = ?",
                        Long.class,
                        municipalidadId);
        return total == null ? -1 : total;
    }
