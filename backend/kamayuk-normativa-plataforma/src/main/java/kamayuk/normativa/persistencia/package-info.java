 ...
 *
 * <p><b>Por que cuelga de {@code kamayuk.normativa} y no de {@code
 * kamayuk.normativa.plataforma}</b>, aunque viva en el mismo modulo Gradle: por el mismo motivo que
 * {@code kamayuk.normativa.dominio}. Para Spring Modulith un subpaquete es interno a su modulo, y
 * un patron que los doce contextos tienen que extender no puede ser interno a nadie. Como {@code
 * compartido} y {@code dominio}, se declara modulo compartido en {@code KamayukAplicacion}: no es
 * un contexto acotado, y que cualquier contexto lo use no es una violacion de los limites sino su
 * proposito.
 *
...
package kamayuk.normativa.persistencia;
