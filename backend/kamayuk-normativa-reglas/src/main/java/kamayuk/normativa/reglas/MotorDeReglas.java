...
public final class MotorDeReglas {
    ...
    /**
     * Un motor para un ambito, y solo para el (ADR-0024).
     *
     * <p>El catalogo se revisa <b>aqui</b>, al construir, y no dentro de {@code aplicarA}: una
     * regla de obligacion colada en una corrida de valuacion no revienta —produce una cifra
     * plausible—, y comprobarlo por partida dejaria el fallo para la mitad de un padron de
     * trescientos mil predios, con la otra mitad ya escrita.
     *
     * @throws ReglaDeOtroAmbito si alguna regla del catalogo es del ambito contrario
     */
    public MotorDeReglas(CatalogoDeReglas catalogo, Ambito ambito) {
        this.catalogo = Objects.requireNonNull(catalogo, "El motor necesita su catalogo de reglas");
        this.ambito =
                Objects.requireNonNull(
                        ambito,
                        "El motor necesita su ambito: sin el, una regla de obligacion se puede"
                                + " aplicar en una corrida de valuacion sin que nada lo diga"
                                + " (ADR-0024)");
        verificarQueTodasSonDelAmbito();
    }

    /** El ambito con el que se construyo. */
    public Ambito ambito() {
        return ambito;
    }

    private void verificarQueTodasSonDelAmbito() {
        for (ReglaTributaria regla : catalogo.todasLasReglas()) {
            if (regla.ambito() != ambito) {
                throw new ReglaDeOtroAmbito(regla.identificador(), regla.ambito(), ambito);
            }
        }
        for (ReglaDeAgregacion regla : catalogo.todasLasAgregaciones()) {
            if (regla.ambito() != ambito) {
                throw new ReglaDeOtroAmbito(regla.identificador(), regla.ambito(), ambito);
            }
        }
    ...
    /**
     * El catalogo trae una regla del ambito contrario (ADR-0024).
     *
     * <p>Se lanza al <b>construir</b> el motor, que es lo que hace este error barato: sale al
     * arrancar el proceso —o al montar el bean— y no a mitad de una emision.
     */
    public static final class ReglaDeOtroAmbito extends RuntimeException {
        @java.io.Serial private static final long serialVersionUID = 1L;

        ReglaDeOtroAmbito(IdentificadorDeRegla regla, Ambito suyo, Ambito delMotor) {
            super(
                    "La regla "
                            + regla
                            + " es de ambito "
                            + suyo
                            + " y este motor corre en "
                            + delMotor
                            + ". Aplicarla aqui no produciria ningun error: produciria una cifra"
                            + " plausible y equivocada sobre todo el padron (ADR-0024)");
        }
    }
