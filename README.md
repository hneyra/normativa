...
| Su frontend (`normativa-web`, ADR-0030 §1) | **La V6 salió ([#50](https://github.com/hneyra/normativa/issues/50)): hoy es una raíz vacía.** La interfaz de cinco secciones que [#39](https://github.com/hneyra/normativa/issues/39) empaquetó y autenticó —con 876 pruebas— se lee en `c01fe9a` (`git show c01fe9a:frontend/<ruta>`), y lo que viene es la épica [#47](https://github.com/hneyra/normativa/issues/47): la misma interfaz y el mismo stack que `rentas`, sobre `kamayuk-lib`. En `frontend/` queda la cadena de herramientas de `rentas@ac379ac`, sin librería todavía; `yarn verificar` en verde con 90 pruebas. `frontend/Dockerfile` + `nginx.conf` siguen produciendo `kamayuk-normativa-interfaz`, que **sirve una página en blanco hasta #58** |
