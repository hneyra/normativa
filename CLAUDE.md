...
| Su frontend (`normativa-web`) | **La V6 salió entera ([#50](https://github.com/hneyra/normativa/issues/50)) y desde [#55](https://github.com/hneyra/normativa/issues/55) `frontend/` monta el `Armazon` de `@kamayuk/shell`, vacío: sin una sola hoja.** Su fuente congelada es `c01fe9a` —se lee con `git show c01fe9a:frontend/<ruta>`— y tenía 90 archivos en `src/` y **876 pruebas**; la reconstrucción con la interfaz de `rentas` sobre `kamayuk-lib` es la épica [#47](https://github.com/hneyra/normativa/issues/47). En `frontend/` está la cadena de herramientas de `rentas@ac379ac` **versión a versión y ya entera** —React 19.3.0, Vite 7, Tailwind 4, TanStack Query, i18next, Playwright…—, con **los cinco `@kamayuk/{api,formato,sesion,shell,ui}` por `link:` al clon hermano** y los tres remiendos de `rentas`: `preserveSymlinks`, `resolucion.ts` (el `dedupe` derivado de las `peerDependencies`) en `vite.config.ts` **y** en `vitest.config.ts`, y `src/estilos.css` con el `@import` de `@kamayuk/ui` y los dos `@source`. `@kamayuk/verificaciones` **sigue fuera** hasta #62. `src/aplicacion.tsx` —**el único archivo que #55 se reserva**— monta `ProveedorDeTema` + `Armazon` con catálogo vacío, textos por omisión, **sin escudo** y **sin opciones de sesión**, y todo lo que recibe sale de **ocho costuras** (`marca`, `sesion`, `catalogo`, `pantallas`, `acciones`, `i18n/armazon`, `datos/proveedor`, `arranque`) para que #57, #58, #60, #63 y #64 no se pisen. El título es el que decidió **G2** —«Sistema de Gestión de Rentas y Tributos Municipales»— y **la municipalidad y la cuenta salen de la sesión, nunca como literal**. `yarn verificar` en verde con **183 pruebas en 20 archivos**; `yarn build` produce `dist/` bajo `/normativa/`. Las prohibiciones siguen siendo **las diez propias**, con la décima —**ninguna cifra tributaria literal en el código**— que no está en ningún otro frontend, hasta #62. **Desde #55 la imagen se construye en cada PR** (`frontend.yml`, con `--build-context kamayuk-lib=…`) y sólo se publica en `main`; **sirve el `Armazon` sin hojas hasta #58**, y conserva su negativa a construirse si lo servido lleva una cifra del corpus. **Y desde [#61](https://github.com/hneyra/normativa/issues/61) hay ARNÉS DE NAVEGADOR, que la V6 nunca tuvo**: Playwright contra el **paquete construido** —`vite preview`, no `yarn dev`—, un job `arnes` propio en `frontend.yml` con `needs: verificar`, `timeout-minutes: 20`, un paso que **avisa al 75 %** de ese tope y el informe subido siempre. El puerto **no se escribe**: `puerto-del-arnes.mjs` lo deriva del árbol por `sha256` (`/home/jorge/ws/normativa/frontend` → **4686**), en CI es el 4173, y si está ocupado falla **antes de construir** diciendo quién lo tiene. Nueve specs, **48 caminos en ~1,2 min**, y cada uno cubre una lección: `los-cuatro` (los cuatro destinos abren por su hash en el bundle, con caja y sin errores de consola), `se-ve` (color, radio, tipografía y rejilla **computados**, y ninguna tabla que desplace la página), `los-temas-llegan-al-navegador` (las **ocho** paletas servidas y pintadas, y que lo guardado lleve el prefijo `kamayuk.normativa`), `la-puerta-caida` (el emisor apagado: monta y dice quién falló y en qué URL), `la-cuenta-la-lleva-el-emisor` (perfil y contraseña van a la consola de Keycloak), **`la-puerta`** —heredera de `c01fe9a:src/api/identidad.test.ts`: `redirect_uri` a `<origin>/normativa/`, S256 comprobado contra el verificador, `kamayuk-backoffice`, `openid profile`, el token **fuera de los dos almacenamientos** y un `configuracion.js` servido distinto que cambia el emisor sin reconstruir—, **`sin-origen-seguro`** (servida por un nombre que no es `localhost` **no redirige y lo dice**, en vez del `TypeError` de la criptografía que falta), `la-paleta-con-teclado` (Ctrl-K abre, filtra, elige y navega sin ratón) y `nada-deshabilitado-sin-motivo` (ningún control impedido sin motivo accesible, y ninguno cortado a 1 440 px). **No corre dentro de `yarn verificar`**, a propósito: necesita un navegador de 114 MB y construir el bundle |
...
`frontend/package.json` enlaza los cinco `@kamayuk/*` por `link:../../kamayuk-lib/paquetes/*`, que
yarn resuelve contra el disco. **`yarn install` sale con código 0 aunque el clon falte** y no
enlaza nada —ni `--check-files` lo caza—: el primer paso que lo dice es cargar `vite.config.ts` o
`vitest.config.ts` —`yarn dev`, `yarn build`, `yarn test`—, y lo dice nombrando el `git clone`.
