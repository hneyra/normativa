...
| Su frontend (`normativa-web`) | **La V6 salió entera ([#50](https://github.com/hneyra/normativa/issues/50)), y lo que queda es una raíz vacía.** Su fuente congelada es `c01fe9a` —se lee con `git show c01fe9a:frontend/<ruta>`— y tenía 90 archivos en `src/` y **876 pruebas**; la reconstrucción con la interfaz de `rentas` sobre `kamayuk-lib` es la épica [#47](https://github.com/hneyra/normativa/issues/47). En `frontend/` queda la cadena de herramientas de `rentas@ac379ac` **versión a versión** —React 19.3.0, Vite 7, Tailwind 4, TanStack Query, i18next, Playwright…—, todavía **sin ningún `@kamayuk/*`** (los cinco de ejecución entran con #55, `@kamayuk/verificaciones` con #62), y `src/main.tsx` monta React sobre `#raiz` sin dibujar nada. `yarn verificar` en verde con **90 pruebas en 5 archivos**; `yarn build` produce `dist/` bajo `/normativa/`. Las prohibiciones siguen siendo **las diez propias**, con la décima —**ninguna cifra tributaria literal en el código**— que no está en ningún otro frontend, hasta #62. Lo vigilan `la-v6-no-esta` y `el-stack-es-el-de-rentas`. **La imagen `kamayuk-normativa-interfaz` que publique `main` sirve una raíz vacía hasta #58**, y conserva su negativa a construirse si lo servido lleva una cifra del corpus |
